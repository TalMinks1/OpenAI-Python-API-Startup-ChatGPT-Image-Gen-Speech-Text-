...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.recording_switches = []
        self.recording_buttons = []
        self.recall = customtkinter.IntVar()
        self.recall1 = customtkinter.IntVar()
    ...
    def recorder(self):
        ...
        switch = customtkinter.CTkSwitch(master=self.recordings_frame, text=f"Recording {i}", onvalue=i, offvalue=i * 100000, variable=self.recall, command=self.recall_recording_text)
        switch.grid(row=i, column=0, pady=(0, 20))
        self.recording_switches.append(switch)

        playback = customtkinter.CTkRadioButton(master=self.recordings_frame, text="▶️", value=i, variable=self.recall1, command=self.recall_recording)
        playback.grid(row=i, column=1, padx=(0, 20), pady=(0, 20))
        self.recording_buttons.append(playback)

    def recall_recording_text(self):
        file_num = self.recall.get()
        if file_num > 100000:
            print("Turned switch off.")

        else:
            if self.translate.get() == 1:
                audio_file = open(f"myrecording{file_num}.wav", "rb")
                transcript = openai.Audio.translate("whisper-1", audio_file)
                self.audio_complete.insert(1.0, text=transcript["text"])
            else:
                audio_file = open(f"myrecording{file_num}.wav", "rb")
    ...
    def recall_recording(self):
        file_num = self.recall1.get()
        playsound(f"myrecording{file_num}.wav")
